...
class train():
    def __init__(self, opts, RNN, RNN_optim, criterion, data_loader, checkpoint_path = './model/checkpoint_-1.pth'):
        ...
        for e in range(self.opts.epoch - last_epoch):
            ...
            for data, labels, lengths in self.data_loader:
                ...
                if steps % self.opts.print_every == 0:
                    print('Epoch: {}, Steps: {}, Loss: {:.4}, Train Accuracy {:4}, '.format(e, steps, loss.item(), correct / float(total)))
                    correct = 0
                    total = 0
