...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(embed_size, self.hidden_size, dropout=dropout_rate, bidirectional=True)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Yt in torch.split(Y, 1, dim=0):
            squeezed = torch.squeeze(Yt)
            ybart = torch.cat((squeezed, o_prev), dim=1)
            dec_state, o_t, _ = self.step(ybart, dec_state, enc_hiddens, enc_hidden_proj, enc_masks)
