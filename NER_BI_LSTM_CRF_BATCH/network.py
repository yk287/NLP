...
class RNN(nn.Module):
    def __init__(self, opts, word_dict, label_dict, output_size=None):
        ...
        #the model
        if self.opts.use_features:
            input_size = self.opts.embedding_dim + self.opts.feature_size
        else:
            input_size = self.opts.embedding_dim

        self.rnn = nn.LSTM(input_size=input_size, hidden_size=self.opts.hidden_size, num_layers=self.opts.num_layers, dropout=self.opts.dropout, bidirectional=True)
        ...
        """
        if self.opts.use_features:
            input_size = self.opts.hidden_size * 2 + self.opts.feature_size
        else:
            input_size = self.opts.hidden_size * 2
        """
        input_size = self.opts.hidden_size * 2

        self.linear = nn.Sequential(
            nn.Linear(input_size, input_size // 2),
            #nn.BatchNorm1d(self.opts.batch_size),
    ...
    def lstm_outputs(self, batch, len):
        ...
        embeds = self.embedding(batch[:, :, 0].to(device))

        if self.opts.use_features:
            features = batch[:,:, 1:batch.shape[2]].float().to(device)
            embeds = torch.cat([embeds, features], dim=2)
        ...
        """
        if self.opts.use_features:
            output = torch.cat([output, features[:output.shape[0],:,:]], dim=2)
        """
    ...
    def get_tags(self, batch, len):
        ...
        mask = (batch[:, :, 0] > self.word_dict['PAD']).float()
