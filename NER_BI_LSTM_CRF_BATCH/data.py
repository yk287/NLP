...
class dataloader():
    ...
    def read_data(self, filename):
        ...
        return OrderedDict(((idx, Sentence(*zip(*[l.strip().split() for l in s]))) for idx, s in enumerate(sentence_lines) if s[0]))
...
class SeqDataset(Dataset):
    ...
    def __init__(self, data_tensor, target_tensor, length_tensor):
        ...
        assert data_tensor.size(1) == target_tensor.size(0) == length_tensor.size(0)
