...
def create_dataset(data, inputindx, targetindx, opts):
    ...
    paddend_label = pad_data(vectorized_label, input_len)

    #label_tensor = torch.LongTensor([targetindx[label] for label in data[1]])

    return DataLoader(SeqDataset(paddend_input, paddend_label, input_len), batch_size=opts.batch_size, shuffle=opts.shuffle, num_workers=opts.cpu_count)
...
def sort_batch(data, targets, lengths):
    ...
    return sorted_data.transpose(0, 1), target_tensor.transpose(0, 1), length
